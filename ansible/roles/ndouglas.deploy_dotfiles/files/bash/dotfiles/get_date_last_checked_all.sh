...
dot_get_date_last_checked_all() {
  ...
  (
    dot_get_date_last_checked_all_file_handler() {
      ...
      response="$(ssh "${user_at_hostname}" -o ConnectTimeout=10 'cat ~/.dotfiles/.last_checked')";
      echo -e "${padded_prompt}:\t${response}";
    }
    export -f dot_get_date_last_checked_all_file_handler;
    parallel --jobs 20 dot_get_date_last_checked_all_file_handler ::: "${authorized_key_path}"/*.pub;
  );
}
